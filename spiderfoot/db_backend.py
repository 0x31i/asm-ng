...
# -------------------------------------------------------------------------------
# Name:         db_backend
# Purpose:      Database backend for PostgreSQL.
#
# Created:      2026-02-19
# Licence:      MIT
# -------------------------------------------------------------------------------

"""PostgreSQL database backend layer.

Provides the connection factory and helpers used by ``SpiderFootDb``.
...
# ---------------------------------------------------------------------------
# psycopg2 import — hard requirement
# ---------------------------------------------------------------------------
import psycopg2
...
# ---------------------------------------------------------------------------
# Unified exception types (PostgreSQL only).
# ---------------------------------------------------------------------------
...
def _attempt_pg_auto_setup(opts: dict) -> bool:
    ...
    if not os.path.isfile(script_path):
        log.error(
            ...
            "PostgreSQL is required — install it manually."
        )
...
def detect_db_type(opts: dict) -> str:
    ...
    Always returns ``'postgresql'``. Retained for API compatibility with
...
def raw_execute(conn, query, params=None):
    ...
    Converts ``?`` to ``%s`` for psycopg2 connections.
